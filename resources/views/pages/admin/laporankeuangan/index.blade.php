 ...
 <div class="main-content">
      ...
      <div class="card-heade">
        ...
        <form action="{{ route('laporan-keuangan.index') }}" method="GET" enctype="multipart/form-data">
            ...
            <select name="kelascari" id="" style="margin-left: 15px" class="form-control" value="{{ old('kelascari') }}"   required>
         ...
         <form action="{{ route('cetakkeuangan') }}" target="_blank" method="GET" enctype="multipart/form-data">
                          @csrf
                        <input type="hidden" name="tahun" class="form-control" value="{{ $tahun }}" required>
                        <input type="hidden" name="kelascari" class="form-control" value="{{ $kelascari }}" required>
                                           
                      <button style="margin-left: 60px" class="btn btn-icon icon-left btn-success"><i class="fa fa-print"></i> Cetak Laporan Keuangan</button>
                    </form>
         </div>
        ...
        <div class="table-responsive">
            ...
            ?>
                ...
                <tr>
                    ...
                    <td>
                      @foreach($k as $urai)
                              @if($urai->rincianpembayaran->uraian_pembayaran=="SEMESTER")
                              @php
                                  $total += $urai->rincianpembayaran->nominal;
                              @endphp
                              {{ number_format($urai->rincianpembayaran->nominal) }}
                    ...
                    <td>
                      @foreach($k as $urai)
                            @if($urai->rincianpembayaran->uraian_pembayaran=="UN")
                            @php
                                $total += $urai->rincianpembayaran->nominal;
                            @endphp
                            {{ number_format($urai->rincianpembayaran->nominal) }}
